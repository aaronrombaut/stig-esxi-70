...
#    Get-VM $myVM | Get-AdvancedSetting -Name $advancedSetting | Format-Table -AutoSize
    ...
    if (Get-VM $myVM | Get-AdvancedSetting -Name $advancedSetting) {
        # "Exists"
        Get-VM $myVM | Get-AdvancedSetting -Name $advancedSetting | Set-AdvancedSetting -Value 1048576 -Confirm:$false
    } else {
        # "Does not exist"
        Get-VM $myVM | New-AdvancedSetting -Name $advancedSetting -Value 1048576 -Confirm:$false
        Get-VM $myVM | Get-AdvancedSetting -Name $advancedSetting | Set-AdvancedSetting -Value 1048576 -Confirm:$false
    }
}